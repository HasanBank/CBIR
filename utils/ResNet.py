...
class ResNet50Hashing(nn.Module):
    def __init__(self, hashLength = 16, numberOfClass = 31):
        super().__init__()
        ...
        self.FCHashed = nn.Linear(2048, hashLength)
        self.FC_Class = nn.Linear(hashLength, numberOfClass)
    ...
    def forward(self, x):
        ...
        hashCode = self.FCHashed(x)
        classCode = self.FC_Class(hashCode)
        
        hashCode = torch.sign(hashCode)
        binary_hashCode = torch.relu(hashCode)
                
        return binary_hashCode, classCode    
